...
class AddTodoBloc extends Bloc<AddTodoEvent, AddTodoState> {
  ...
  AddTodoBloc(this._useCase) : super(AddTodoState()) {
    on<AddTaskEvent>(addTodo);
  }
  Future<void>addTodo(AddTaskEvent event, Emitter<AddTodoState> emit)async{
     emit(state.copyWith(status: Status.loading));
      final result = await _useCase.call(TodoModelParams(todoModel: event.todoModel));
      result.fold((failure) {
        emit(state.copyWith(status: Status.error, failure: failure));
      }, (id) {
        emit(state.copyWith(status: Status.success, todoModel: event.todoModel));
      });
  }
}
